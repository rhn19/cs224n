...
def question_1h_sanity_check():
    ...
    print ("-"*80)
    high = Highway(EMBED_SIZE)
    x_conv = torch.randn(BATCH_SIZE, EMBED_SIZE)
    highway = high(x_conv)
    #print(highway.size())
    expected_output_size = [BATCH_SIZE, EMBED_SIZE]
    assert(list(highway.size()) == expected_output_size), "Shape Mismatch. It should be:\n{} but is:\n{}".format(expected_output_size, list(highway.size()))
...
def question_1i_sanity_check():
    ...
    print ("-"*80)
    num_filters = EMBED_SIZE #as specified in pdf
    char_embed_size = 40
    max_word_length = 10
    inpt = torch.randn(BATCH_SIZE, char_embed_size, max_word_length)
    conv = CNN(num_filters, char_embed_size, max_word_length, 2)
    conv_out = conv(inpt)
    expected_output_size = [BATCH_SIZE, EMBED_SIZE]
    assert(list(conv_out.size()) == expected_output_size), "Shape Mismatch. It should be:\n{} but is:\n{}".format(expected_output_size, list(conv_out.size()))
