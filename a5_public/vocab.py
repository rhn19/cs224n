...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        w2cidx = self.words2charindices(sents)
        padded = pad_sents_char(w2cidx, char_pad_token=self.char2id['<pad>'])
        sents_var = torch.LongTensor(padded).permute(1,0,2).to(device)
        return sents_var
